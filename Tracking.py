...
class ObjectTracking:
    ...
    def __init__(self, model="yolo26n.pt", source=0): # Insert path to camera apparently 0 refers to standard cam (something with cv2)
    ...
    def draw_bbox(self, im0, box, track_id, cls):
        ...
        # Prepare label
        label = f"{self.names[int(cls)]}:{int(track_id)}" # where do labels go?
...
if __name__ == "__main__":
    ...
    tracker = ObjectTracking(
        model="yolo26n.pt",
        source=0 # Insert path to camera. 0 is default???, check opencv for docs.
    )
    tracker.run()
